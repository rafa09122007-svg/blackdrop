...
function Label({ text, required }) {
  return (
    <div style={{ color: T.muted, fontSize: 10, fontWeight: 600, letterSpacing: "0.12em", textTransform: "uppercase", marginTop: 14, marginBottom: 5 }}>
...
function Logo({ sub }) {
  return (
    <div style={{ textAlign: "center", marginBottom: 20 }}>
      <div style={{ color: T.gold, fontSize: 10, fontWeight: 700, letterSpacing: "0.4em", fontFamily: "'Rajdhani',sans-serif" }}>BLACK DROP TRUCKING</div>
      <div style={{ color: T.gold, fontSize: 21, fontWeight: 700, letterSpacing: "0.18em", fontFamily: "'Rajdhani',sans-serif", marginTop: 3 }}>{sub}</div>
...
function Login({ onLogin }) {
  ...
  async function handleLogin() {
    ...
    try {
      ...
      if (list.some((p) => String(p).trim() === phone.trim())) onLogin(phone.trim());
...
function Queue({ phone, onEdit, onBack }) {
  ...
  const counts = useMemo(() => ({ total: tickets.length, pending: tickets.filter((t) => t["Status"] === "PENDING").length, bounce: tickets.filter((t) => t["Status"] === "BOUNCE BACK").length, approved: tickets.filter((t) => t["Status"] === "APPROVED").length }), [tickets]);
  ...
  return (
    <PageShell maxW={520}>
      ...
      {loading && <div style={{ textAlign: "center", padding: 40, display: "flex", flexDirection: "column", alignItems: "center", gap: 12, color: T.muted }}><Spinner size={24} /> Loading queue...</div>}
      {error && <div style={{ color: T.danger, textAlign: "center", padding: 32 }}>⚠ Failed to load. Check your connection.</div>}
      {!loading && !error && tickets.length === 0 && <div style={{ color: T.muted, textAlign: "center", padding: 40 }}>No submissions found for this number.</div>}
        ...
        return (
          <div key={ticket["Submission ID"] || i} style={{ background: T.surface, borderRadius: 10, padding: 14, marginBottom: 10, border: `1px solid ${isBounce ? "rgba(239,68,68,0.3)" : T.border}`, borderLeft: `3px solid ${isBounce ? T.danger : status === "APPROVED" ? T.success : T.warn}` }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "6px 12px" }}>
              {[["Client", ticket["Client"]], ["Created", ticket["Timestamp"] ? new Date(ticket["Timestamp"]).toLocaleString() : "—"], ticket["Field Ticket #"] ? ["Field Ticket", ticket["Field Ticket #"]] : null, ticket["Driver"] ? ["Driver", ticket["Driver"]] : null].filter(Boolean).map(([k, v]) => (
                <div key={k}><div style={{ color: T.muted, fontSize: 10 }}>{k}</div><div style={{ color: T.text, fontSize: 12 }}>{v}</div></div>
              ))}
            </div>
...
function SubmitTicket({ phone, onComplete, editTicket, onBack }) {
  ...
  const defaultForm = useMemo(() => {
    ...
    return { client: "", fieldTicket: "", dispatch: "", unit: "", driver: "", workDate: today, wellLease: "", notes: "", fieldTicketImage: "", startTime: "", endTime: "", hourlyRate: "" };
  }, [editTicket, today]);

  const defaultLoads = useMemo(() => {
    if (editTicket) return [{ id: 1, geminiRef: "", loadTicket: "", fluid: "Fresh Water", bbls: "", manifestOps: { washOut: false, unload: false }, verificationImage: "" }];
    const draft = loadDraft();
    if (draft && draft.loads) return draft.loads;
    return [{ id: 1, geminiRef: "", loadTicket: "", fluid: "Fresh Water", bbls: "", manifestOps: { washOut: false, unload: false }, verificationImage: "" }];
  }, [editTicket]);
  ...
  useEffect(() => { if (!editTicket) saveDraft({ form, loads }); }, [form, loads, editTicket]);

  const isExxon = form.client === "Exxon";
  function update(k, v) { setForm((p) => ({ ...p, [k]: v })); }
  function updateLoad(i, k, v) { setLoads((p) => { const c = [...p]; c[i] = { ...c[i], [k]: v }; return c; }); }
  ...
  function toggleOp(i, op) { setLoads((p) => { const c = [...p]; c[i] = { ...c[i], manifestOps: { ...c[i].manifestOps, [op]: !c[i].manifestOps[op] } }; return c; }); }
  const totalBBLS = useMemo(() => loads.reduce((s, l) => s + (parseFloat(l.bbls) || 0), 0), [loads]);
  ...
  const timeCalc = useMemo(() => {
    ...
    const parts1 = form.startTime.split(":"); const parts2 = form.endTime.split(":");
    if (parts1.length < 2 || parts2.length < 2) return null;
    const sh = parseInt(parts1[0], 10); const sm = parseInt(parts1[1], 10); const eh = parseInt(parts2[0], 10); const em = parseInt(parts2[1], 10);
    if (isNaN(sh) || isNaN(sm) || isNaN(eh) || isNaN(em)) return null;
    let mins = eh * 60 + em - (sh * 60 + sm); if (mins < 0) mins += 1440; if (mins === 0) return null;
    const hrs = mins / 60; const rate = parseFloat(form.hourlyRate);
  ...
  function pt(e, canvas) { const r = canvas.getBoundingClientRect(); const cx = e.touches ? e.touches[0].clientX : e.clientX; const cy = e.touches ? e.touches[0].clientY : e.clientY; return { x: (cx - r.left) * (canvas.width / r.width), y: (cy - r.top) * (canvas.height / r.height) }; }
  function startDraw(e) { if (e.type === "touchstart") e.preventDefault(); const c = sigRef.current; if (!c) return; const ctx = c.getContext("2d"); drawing.current = true; const p = pt(e, c); ctx.lineWidth = 2.5; ctx.lineCap = "round"; ctx.lineJoin = "round"; ctx.strokeStyle = "#000"; ctx.beginPath(); ctx.moveTo(p.x, p.y); }
  function draw(e) { if (!drawing.current) return; if (e.type === "touchmove") e.preventDefault(); const c = sigRef.current; if (!c) return; const ctx = c.getContext("2d"); const p = pt(e, c); ctx.lineTo(p.x, p.y); ctx.stroke(); setHasSignature(true); }
  function endDraw() { drawing.current = false; }
  function clearSig() { const c = sigRef.current; if (!c) return; c.getContext("2d").clearRect(0, 0, c.width, c.height); setHasSignature(false); }

  async function handleSubmit() {
    if (!isComplete || isSubmitting) return;
    setIsSubmitting(true); setSubmitError("");
    const sigCanvas = sigRef.current;
    const payload = { submissionId, phone, ...form, loads, totalBBLS, signature: sigCanvas ? sigCanvas.toDataURL("image/png") : "" };
    try {
      ...
      try { const q = JSON.parse(sessionStorage.getItem("offlineTickets") || "[]"); q.push(payload); sessionStorage.setItem("offlineTickets", JSON.stringify(q)); alert("No internet — ticket saved offline and will sync when reconnected."); clearDraft(); onComplete(); } catch { setSubmitError("Submission failed. Please try again."); }
    } finally { setIsSubmitting(false); }
  }

  useEffect(() => {
    async function syncOffline() {
      const raw = sessionStorage.getItem("offlineTickets"); const q = raw ? JSON.parse(raw) : []; if (!q.length) return;
      const remaining = [];
      for (const t of q) { try { await fetch(API_URL, { method: "POST", mode: "no-cors", headers: { "Content-Type": "text/plain;charset=utf-8" }, body: JSON.stringify(t) }); } catch { remaining.push(t); } }
      if (remaining.length > 0) sessionStorage.setItem("offlineTickets", JSON.stringify(remaining)); else sessionStorage.removeItem("offlineTickets");
    }
    window.addEventListener("online", syncOffline); syncOffline();
  ...
  return (
    <PageShell maxW={520}>
      ...
      <div style={S}>
        ...
        <Label text="Client Organization" required /><Select value={form.client} onChange={(e) => update("client", e.target.value)}><option value="">Select client…</option>{["Exxon", "Oxy", "Western Midstream", "Chevron", "Other"].map((c) => (<option key={c}>{c}</option>))}</Select>
        <div style={G2}><div><Label text="Field Ticket #" required /><Input value={form.fieldTicket} onChange={(e) => update("fieldTicket", e.target.value)} /></div><div><Label text={dispatchLabel} required /><Input value={form.dispatch} onChange={(e) => update("dispatch", e.target.value)} /></div></div>
        <div style={G2}><div><Label text="Unit / Truck #" required /><Input value={form.unit} onChange={(e) => update("unit", e.target.value)} /></div><div><Label text="Driver Name" required /><Input value={form.driver} onChange={(e) => update("driver", e.target.value)} /></div></div>
        <div style={G2}><div><Label text="Work Date" required /><Input type="date" value={form.workDate} onChange={(e) => update("workDate", e.target.value)} /></div><div><Label text="Well / Lease" required /><Input value={form.wellLease} onChange={(e) => update("wellLease", e.target.value)} /></div></div>
...
export default function App() {
  injectGlobalStyles();
  const [phone, setPhone] = useState(() => { try { return sessionStorage.getItem("bd_phone"); } catch { return null; } });
  const [page, setPage] = useState(() => { try { return sessionStorage.getItem("bd_phone") ? "dashboard" : "login"; } catch { return "login"; } });
  const [editTicket, setEditTicket] = useState(null);

  function login(p) { try { sessionStorage.setItem("bd_phone", p); } catch {} setPhone(p); setPage("dashboard"); }
  function logout() { try { sessionStorage.removeItem("bd_phone"); clearDraft(); } catch {} setPhone(null); setPage("login"); }
  ...
  if (page === "submit") return <SubmitTicket phone={phone} editTicket={editTicket} onComplete={() => { setEditTicket(null); clearDraft(); setPage("success"); }} onBack={() => setPage("dashboard")} />;
